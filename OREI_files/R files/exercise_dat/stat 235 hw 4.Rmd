...
X1_range <- seq(from=26, to=86,by =1)
RW_logit <- -4.78 + 0.0671* X1_range
DW_logit <- -7.99 + 0.1137* X1_range
DS_logit <- -8.13 + 0.1146* X1_range
 
# Compute the probibilities (this is what will actually get plotted):
RW_probs <- exp(RW_logit)/(1 + exp(RW_logit))
DW_probs <- exp(DW_logit)/(1 + exp(DW_logit))
DS_probs <- exp(DS_logit)/(1 + exp(DS_logit))
RS_probs <- 1 - (RW_probs + DW_probs + DS_probs)
 
library(ggplot2); library(tidyr)
# first you have to get the information into a long dataframe, which is what ggplot likes :)
plot.data <- data.frame(RepStrong=RS_probs, RepWeak=RW_probs, DemWeak=DW_probs, DemStrong=DS_probs, X1=X1_range)
plot.data <- gather(plot.data, key=group, value=prob, RepStrong:DemStrong)
head(plot.data)
 
ggplot(plot.data, aes(x=X1, y=prob, color=group)) + # asking it to set the color by the variable "group" is what makes it draw 4 different lines
  geom_line(lwd=1) +
  scale_x_continuous(limits = c(20, 90)) +
  scale_color_manual(values = c("darkcyan","coral1","darkmagenta", "firebrick")) +
  labs(x="college percentage of county", y="probability of vote type", title="prob. of county vote type ~ percentage of people with some college education") +
  theme_bw()
```
...
# Compute the probibilities (this is what will actually get plotted):
RW_probs <- exp(RW_logit)/(1 + (exp(RW_logit)+ exp(DW_logit) + exp(DS_logit)))
DW_probs <- exp(DW_logit)/(1 + (exp(RW_logit)+ exp(DW_logit) + exp(DS_logit)))
DS_probs <- exp(DS_logit)/(1 + (exp(RW_logit)+ exp(DW_logit) + exp(DS_logit)))
RS_probs <- 1 - (RW_probs + DW_probs + DS_probs)
 
library(ggplot2); library(tidyr)
# first you have to get the information into a long dataframe, which is what ggplot likes :)
plot.data <- data.frame(RepWeak=RW_probs, DemWeak=DW_probs, DemStrong=DS_probs, X1=X1_range)
plot.data <- gather(plot.data, key=group, value=prob, RepWeak:DemStrong)
head(plot.data)
 
ggplot(plot.data, aes(x=X1, y=prob, color=group)) + # asking it to set the color by the variable "group" is what makes it draw 4 different lines
  geom_line(lwd=1) +
  scale_x_continuous(limits = c(20, 90)) +
  scale_color_manual(values = c("darkcyan","coral1","darkmagenta")) +
  labs(x="college percentage of county", y="probability of vote type", title="prob. of county vote type ~ percentage of people with some college education") +
  theme_bw()
```
